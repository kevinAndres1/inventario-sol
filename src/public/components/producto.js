...
// componente clients
let producto = Vue.component('producto', {
  ...
  data: () => ({
    ...
    headers: [
		...
		{ text: 'Entrada unidad', value: 'entradaUnidad' },
		{ text: 'Salida unidad', value: 'salidaUnidad' },
  ...
  computed: {
    formTitle () {
      return this.editedIndex === " " ? 'Registrar Nuevo producto' : 'Actualizar un producto';
    },
  },
  ...
  methods: {
    initialize: async function () {
      this.producto = await execute('muestra-productos');

      if(Math.round ( Object.keys(this.producto).length / 16) >= 1 )
        this.pageCount =  Math.round ( Object.keys(this.producto).length / 16);
    },
    ...
    deleteItemConfirm: async function() {
      let result = await execute('borrar-producto', this.editedIndex);
	...
	generatePDFAll: async function() {
		...
		let result = await execute('generate-pdf-producto', {export_all: 1});
	...
	generatePDF: async function() {
		...
		let result = await execute('generate-pdf-producto', this.pdf);
    ...
    save: async function() {
      ...
      if (this.editedIndex != " ") {
        result = await execute('actualizar-producto', this.editedItem);
      } else {
        result = await execute('crear-producto', this.editedItem);
      }
