...
let otro = Vue.component('otro', {
  ...
  computed: {
    formTitle () {
      return this.editedIndex === " " ? 'Registrar Nuevo suministro' : 'Actualizar un suministro';
    },
  },
  ...
  methods: {
    initialize: async function () {
      this.otros = await execute('muestra-todos-suministros');

      if(Math.round ( Object.keys(this.otros).length / 16) >= 1 )
        this.pageCount =  Math.round ( Object.keys(this.otros).length / 16);
    },

    editItem: async function(item) {
      this.editedIndex = item.codigo
      this.editedItem = await execute('mostrar-otro-suministro', this.editedIndex);
      this.dialog = true
    },
    ...
    deleteItemConfirm: async function() {
      let result = await execute('borrar-otro-suministro', this.editedIndex);
	...
	generatePDFAll: async function() {
		...
		let result = await execute('generate-pdf-otro-suministro', {export_all: 1});
	...
	generatePDF: async function() {
		...
		let result = await execute('generate-pdf-otro-suministro', this.pdf);
    ...
    save: async function() {
      ...
      if (this.editedIndex != " ") {
        result = await execute('actualizar-otro-suministro', this.editedItem);
      } else {
        result = await execute('crear-otro-suministro', this.editedItem);
      }
	...
	template: `
  		<v-container>
			...
			>
  				<template v-slot:top>
    				<v-toolbar flat >
      					<v-toolbar-title>Otros suminitros</v-toolbar-title>
						<v-divider
							class="mx-4"
							inset
							vertical
