...
// componente clients
let pintura = Vue.component('pintura', {
  ...
  computed: {
    formTitle () {
      return this.editedIndex === " " ? 'Registrar Nueva Pintura' : 'Actualizar una Pintura';
    },
  },
  ...
  methods: {
    initialize: async function () {
      this.pinturas = await execute('muestra-pinturas');

      if(Math.round ( Object.keys(this.pinturas).length / 16) >= 1 )
        this.pageCount =  Math.round ( Object.keys(this.pinturas).length / 16);
    },

    editItem: async function(item) {
      this.editedIndex = item.codigo
      this.editedItem = await execute('mostrar-pintura', this.editedIndex);
      this.dialog = true
    },
    ...
    deleteItemConfirm: async function() {
      let result = await execute('borrar-pintura', this.editedIndex);
	...
	generatePDFAll: async function() {
		...
		let result = await execute('generate-pdf-pintura', {export_all: 1});
	...
	generatePDF: async function() {
		...
		let result = await execute('generate-pdf-pintura', this.pdf);
    ...
    save: async function() {
      ...
      if (this.editedIndex != " ") {
        result = await execute('actualizar-pintura', this.editedItem);
      } else {
        result = await execute('crear-pintura', this.editedItem);
      }
	...
	template: `
  		<v-container>
			...
			>
  				<template v-slot:top>
    				<v-toolbar flat >
      					<v-toolbar-title>Pinturas</v-toolbar-title>
						<v-divider
							class="mx-4"
							inset
							vertical
      ...
      <v-dialog v-model="dialogDelete" max-width="600px">
        <v-card>
          <v-card-title class="text-h5">Estas seguro que deseas eliminar esta Pintura?</v-card-title>
          <v-card-actions>
            <v-spacer></v-spacer>
